...
/*  CSS:
      - added "transform" attributes to "mover" class

  JS:
      - "changePizzaSizes" function optimized
      - variables declared inside "for" loops that are used with the same value over and over has been taken out
      - "querySelectorAll" replaced for "getElementsByClassName"
      - "querySelector" replaced for "getElementById"
      - phase calculation improved
      - number of background pizzas calculated dinamically for each device's width and height

  HTML:
      - optimized pizzeria.jpg image (size reduced)
*/
/*
...
var resizePizzas = function(size) {
  ...
  /* ***** "changePizzaSizes" function optimized ***** */
  /* it calculates the width in % and the calculation is simplified due it doesn't need
     the previous width to calculate the new one */
  ...
  /* ***** END OF "changePizzaSizes" function optimized ***** */
...
/* ***** variable "pizzasDiv" is now outside of the for loop ***** */
/* ***** "querySelector" replaced for "getElementById" ***** */
...
/* ***** "querySelectorAll" replaced for "getElementsByClassName" ***** */
/* ***** "mover items array created outside the function because it is a static array and we can avoid to
          calculate its value in every cycle"                                                              ***** */
  ...
  for (var i = 0; i < moverItems.length; i++) {
    ...
    */
    ...
    /* Using style.left, is there a more efficient way to change the position of this object?
       ...
       Perhaps CSS3 hardware acceleration can reduce the need trigger a re-layout? Can we offload the CPU and use the GPU (Video card)
       to help us calculate these animations?
       The CSS 'transform' property can help us here.
    */
    moverItems[i].style.left = moverItems[i].basicLeft + 100 * resultPhase + 'px';
  }
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  var cols = Math.round(screenWidth/300)+2; // Add one pizza to the row for each 300 px;
  var rows = Math.round(screenHeight/260); // Add one row for each 266 px;
